...
public class Driver {
    ...
    public Driver() {
        ...
        findOrderButton.setPreferredSize(new Dimension(200, 50));
        findOrderButton.addActionListener(e -> {
            int tableNumber = ThreadLocalRandom.current().nextInt(0, 10);
            Order order = tablesOrderManager.getOrders().get(tableNumber);
            if (order != null) {
                JOptionPane.showMessageDialog(frame, "Order found at table " + tableNumber);
            } else {
        ...
        layout.putConstraint(SpringLayout.WEST, findOrderButton, 25, SpringLayout.WEST, buttonPanel);
        layout.putConstraint(SpringLayout.NORTH, findOrderButton, 30, SpringLayout.SOUTH, addTableOrderButton);

        layout.putConstraint(SpringLayout.WEST, removeOrderButton, 25, SpringLayout.WEST, buttonPanel);
        layout.putConstraint(SpringLayout.NORTH, removeOrderButton, 10, SpringLayout.SOUTH, findOrderButton);
        ...
        buttonPanel.add(findOrderButton);
